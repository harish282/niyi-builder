...
│   ├── blocks/       # Block definitions & renderers
│   ├── animations/
│   ├── forms/
│   ├── history/      # Undo/redo
...
- *(Future: Mailchimp, Brevo, HubSpot)*
...
*Durations are from the spec; calendar dates should be set when work starts.*
...
| 0.2 | WordPress plugin scaffold | Main plugin file, activation, autoload |
| 0.3 | React + TypeScript + Vite in `plugin/` | Dev server, build to `plugin/build` |
...
*(Draft IDs #79–86 = 8 issues in M10; epics #1, #12, etc. are additional tracking issues.)*
...
*Document version: 1.1 — GitHub issues/milestones/labels created, 2026-06-02.*
