...
/**
 * Raw top-level block list without any document/schema interpretation.
 * Used by consumers that need to reason about top-level structure before
 * building a document (e.g. the editor's import path).
 */
export function parseRawBlocks(markup: string): ParsedBlock[] {
...
export function parseFromGutenberg(markup: string, options: ParseOptions = {}): BuilderDocument {
  ...
  idCounter = 0;
  const blocks = normalizeTopLevelBlocks(parse(trimmed));
